...
curl https://dl.bintray.com/openbangla/i/ubuntu.conf | sed "s/@NAME@/$DIST/" | sudo tee /etc/sources.list.d/openbangla.list
sudo apt-get update
```
WARNING: Replace `YOUR_DISTNAME` with your distro's _base ubuntu codename_, eg: `bionic`, `focal`, etc.
...
DIST=YOUR_DISTNAME
curl https://dl.bintray.com/openbangla/i/pub.key | sudo apt-key add -
curl https://dl.bintray.com/openbangla/i/debian.conf | sed "s/@NAME@/$DIST/" | sudo tee /etc/sources.list.d/openbangla.list
sudo apt-get update
```
WARNING: Replace `YOUR_DISTNAME` with your distro's _codename_, eg: `stretch`, `buster`, etc.
